...
| **D14** | **`apps/runtime` is een API-only Next-app (UI eruit gestript), geen losse service. Deploy blijft per fondsinstance (plus tenant zero).** | **geamendeerd (JW, 2026-08-21)** — [ADR-multitenant-database.md](../architecture/ADR-multitenant-database.md) tak B: gedeeld dashboard, geen gedeelde runtime. Database-per-fonds = promotiepad, niet default. | Runtime-eisen die een Next-app niet aankan; **of** CREATE ROLE op de addon (dan pas gedeelde runtime = tak A) |
| **D15** | **tenant = instance/deployment-identiteit; fund = de klant; 1-op-1 per runtime-proces (één tenant per fonds), tenant-zero = het demo-fonds. `fund` blijft domeinwoord, `tenant_id` de technische sleutel. Control plane mag meerdere fondsen kennen; één runtime-proces blijft één fonds.** | **geamendeerd (JW, 2026-08-21)** — zelfde ADR, tak B. Herzieningscriterium “meerdere fondsen op één instance” is **niet** getrokken: CREATE ROLE ontbreekt ([NOTE-db-rollen-en-pooling.md](../architecture/NOTE-db-rollen-en-pooling.md)). | CREATE ROLE / tak A: fail-closed `SET LOCAL ROLE` bewezen, dán D15 collapsen |

...
`apps/dashboard` (admin console), `packages/db` (`tenant_config` + writer role).
...
3. **Runtime theming replaces `[data-fund]` as the product mechanism.** A tenant theme is the curated
   token subset (primary, radius, logo) + NL copy, stored in `tenant_config`, served by `GET /config`,
...
4. **Public tenant-key + CORS, per tenant.** The key is a public identifier, not a secret: it may sit
   ...
   tenant (no `tenant_config` row) stays open + rate-limited — the tenant-zero demo and local dev.
...
6. **Second DB role: `tenant_config_writer`.** The console writes theming / rotates keys through a
   dedicated writer connection (`getWriterDb`, `TENANT_CONFIG_WRITER_DATABASE_URL`), falling back to
   `DATABASE_URL` locally. In deployment it is a DB user granted write on `tenant_config` only, so the
   console holds no broader write access. Same Scalingo caveat as the analytics reader (D4): the role
   is provisioned via the platform, not `CREATE ROLE` in a migration.
...
- The `tenant_config_writer` DB user is provisioned on Scalingo; the app only wires the connection.

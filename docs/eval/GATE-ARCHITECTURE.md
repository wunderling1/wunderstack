...
|---|---|---|---|
| hardHallucination | ≥ 0.98 | rate | `[X]` kernbelofte bewijsbare betrouwbaarheid. **2026-08-29:** één groene run @ `EVAL_GENERATION_SAMPLES=3` (100%, 38/38); de rode run @ samples=2 is niet gearchiveerd. |
...
| slotbeurt stelt nieuwe vraag | trend (geen vloer) | **count** | `[C]` dezelfde reden als `unclosedClosingTurnCount`: N=1 sluitbeurt (`rp-end-005`), temperature 0.7, detector met erkende retorische false positive. Een vloer van 0 is dan een muntworp. Gemeten en in het artefact (`closingQuestionCount`); promoveer tot vloer samen met `unclosedClosingTurnCount` zodra de set ≥ 3 sluitbeurten heeft. CI #155 (push `c94ac81`, 2026-08-26) was de aanleiding — interventielog C4. |
...
| Golden-set-schema | refusal-cases ≥ 1 distractor; loader gooit zonder profile sidecar `fixtures/fund-sets/<key>.json` | E3/E12 |
...
| 2026-08-26 | **Slotvraagvloer gedemoteerd tot trend.** `maxClosingQuestionCount` uit `ROLEPLAY_THRESHOLDS` / `PERSONA_FLOORS`. `closingQuestionCount` blijft in het artefact en in de detailregel van de mismatch-vloer. Promoveer samen met `unclosedClosingTurnCount` tot vloer bij ≥ 3 sluitbeurten. | N=1 sluitbeurt (`rp-end-005`), temperature 0.7, detector met erkende retorische FP. CI #155 (push `c94ac81`) rood op 1 (max 0) terwijl de rest van de run groen was. Zelfde argument als de bestaande trend `unclosedClosingTurnCount`. Interventielog C4. | Cursor/Jordy |

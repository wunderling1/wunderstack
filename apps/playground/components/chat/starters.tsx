...
export function Starters({ tagline, starters, onPick }: StartersProps) {
  return (
    <div className="flex flex-col items-center gap-4 py-10 text-center">
      <p className="text-sm text-text-muted">{tagline}. Ik antwoord met bronvermelding en verzin niets.</p>
      <div className="flex flex-col gap-2">
        {starters.map((question) => (
          <button
            ...
            className="rounded-md border border-border bg-surface px-3 py-2 text-sm text-text-muted transition-colors hover:bg-surface-sunk"
          >
            {question}
          </button>
        ))}

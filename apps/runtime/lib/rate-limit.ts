...
/**
 * A single global daily ceiling on chat requests for the whole process — the tenant-zero
 * denial-of-wallet backstop (Fase 5). It is deliberately coarse: one counter, not per key or IP, so
 * even a distributed flood across many keys/IPs cannot run the demo's inference bill past a known
 * limit for the day. Resets at UTC midnight. `max <= 0` disables it (local/dev default).
 *
 * Same LIMITATION as the counters above: per process. A multi-instance deployment needs a shared
 * store; keep that change confined to this file.
 */
let dailyCount = 0;
let dailyResetAt = 0;
...
export function checkDailyCap(max: number): RateLimitResult {
  if (max <= 0) {
    return { ok: true, retryAfterSeconds: 0 };
  }
  const now = Date.now();
  ...
  dailyCount += 1;
  if (dailyCount > max) {
    return { ok: false, retryAfterSeconds: Math.max(1, Math.ceil((dailyResetAt - now) / 1000)) };
  }
  return { ok: true, retryAfterSeconds: 0 };
}

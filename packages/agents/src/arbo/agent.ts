 ...
 * verified prose or `NOT_IN_CATALOG_MESSAGE`), so a single `text` delta carries only safe text. There is
...
async function maybeSuggestFollowUps(args: {
  ...
  const span = args.trace.startModelCall(`${AGENT_KEY}-follow-ups`, {
...
export function createArboAgent(): GroundedAgent {
  const agent = new Agent({
    id: AGENT_KEY,
    name: AGENT_KEY,
    instructions: ARBO_SYSTEM_INSTRUCTIONS,
  ...
  return {
    ...
    async *answerStream(
      ...
      try {
        ...
        if (retrieval.hits.length === 0) {
          yield { type: "text", delta: NOT_IN_CATALOG_MESSAGE };
          yield {
            type: "citations",
            found: false,
            needsClarification: false,
